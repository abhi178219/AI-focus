...
async function isAllowedByCrawlers(siteUrl: string, userAgent = '*'): Promise<boolean> {
  try {
    const robotsUrl = new URL('/robots.txt', siteUrl).href
    const res = await fetch(robotsUrl, { signal: AbortSignal.timeout(5000) })
    if (!res.ok) return true // If robots.txt missing, assume allowed
...
export async function crawlIndiaSources(): Promise<RawArticle[]> {
  ...
  for (const target of TARGETS) {
    ...
    try {
      ...
      const crawler = new PlaywrightCrawler({
        ...
        async requestHandler({ page }) {
          ...
          const articles = await page.evaluate(
            ({ titleSel, linkSel, excerptSel }: { titleSel: string; linkSel: string; excerptSel?: string }) => {
              const items: Array<{ title: string; url: string; excerpt: string | null }> = []
              ...
              titleEls.forEach((el) => {
                ...
                if (!titleText || !href || !href.startsWith('http')) return
      ...
      console.warn(`Crawl failed for ${target.name}:`, (err as Error).message)
    }
  }
